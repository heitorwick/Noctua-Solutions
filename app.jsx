  ...
  useEffect(() => {
    ...
    const checkScroll = () => {
      ...
      document.querySelectorAll('.reveal:not(.in)').forEach(el => {
        if (el.getBoundingClientRect().top <= vh * 0.93) show(el);
      });
    };

    // IO para deteccao precisa
    let io = null;
    ...
    // Scroll event como backup
    window.addEventListener('scroll', checkScroll, { passive: true });
    window.addEventListener('resize', checkScroll, { passive: true });

    // Check inicial com delay para garantir que o DOM esteja pronto
